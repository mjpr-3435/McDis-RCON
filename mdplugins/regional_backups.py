import asyncio
import zipfile
import json
import time
import os
import re

from datetime import datetime
from mcdis_rcon.utils import hover_and_suggest, extras
from mcdis_rcon.classes import Server

targets = []
waiting = {}
files_to_zip = {}
creating_bkp = False
admins = []

async def load(server: Server):
    global admins

    path_file = os.path.join(server.path_plugins_configs, 'regional_backups.json')
    dictionary = {
        'Admins' : []
        }
    
    os.makedirs(os.path.join(server.path_plugins, 'reg-bkps'), exist_ok = True)

    if not os.path.exists(path_file):
        os.makedirs(server.path_plugins_configs, exist_ok = True)
        with open(path_file, 'w', encoding = 'utf-8') as file:
            json.dump(dictionary, file, ensure_ascii = False, indent = 4)
    
    with open(path_file, 'r', encoding = 'utf-8') as file:
        config = json.load(file)

    admins = config['Admins']

async def on_player_command(server: Server, player: str, message: str):
    global targets, files_to_zip, waiting, creating_bkp
    
    reg_bkps_dir = os.path.join(server.path_plugins, 'reg-bkps')
    zips = [x for x in os.listdir(reg_bkps_dir) if x.endswith('.zip')]

    if not player in admins:
        return
    
    elif server.is_command(message, 'help'):
        server.show_command(player, 'rb help'           , 'Muestra los comandos del regional backup.')

    elif server.is_command(message, 'rb help'):
        server.show_command(player, 'rb clear'          , 'Limpia tu lista.')
        server.show_command(player, 'rb list'           , 'Muestra tu lista de regiones.')
        server.show_command(player, 'rb add'            , 'Añade a tu lista una región.')
        server.show_command(player, 'rb del <index>'    , 'Elimina la región de índice <index> de tu lista.')
        server.show_command(player, 'rb mk-bkp <name>'  , 'Crea un reg-bkp <name>.zip con las regiones añadidas.')
        server.show_command(player, 'rb bkps'           , 'Lista los backups creados.')
        server.show_command(player, 'rb load-bkp <name>', 'Carga el reg-bkp <name>.zip.')
        server.show_command(player, 'rb del-bkp <name>' , 'Elimina el reg-bkp <name>.zip.')

    elif server.is_command(message, 'rb add'):
        server.execute(f'data get entity {player}')
        targets.append(player)
    
    elif server.is_command(message, 'rb del'):
        index = int(message.removeprefix(f'{server.prefix}rb del'))
        if not player in files_to_zip.keys(): 
            server.send_response(player, 'Lista vacía.')
        elif not any(len(files_to_zip[player][dim]) for dim in ['overworld', 'the_nether', 'the_end']): 
            server.send_response(player, 'Lista vacía.')
        else:
            i = 0
            for dim in files_to_zip[player].keys():
                for reg in files_to_zip[player][dim]:
                    i += 1
                    if index == i:
                        files_to_zip[player][dim].remove(reg)
            show_list(server, player)
    
    elif server.is_command(message, 'rb load-bkp'):
        zip = message.removeprefix(f'{server.prefix}rb load-bkp').strip() + '.zip'

        if zip in zips:
            server.stop()

            while server.is_running():
                await asyncio.sleep(0.1)

            await server.send_to_console(f'Unpacking reg-bkp {zip}...')

            source = os.path.join(server.path_plugins, 'reg-bkps', zip)
            destination = os.path.join(server.path_files, 'world')

            with zipfile.ZipFile(source, 'r') as zip_ref:
                for file in zip_ref.namelist():
                    zip_ref.extract(file, destination)

            await server.send_to_console(f'Reg-bkp {zip} unpacked.')

            server.start()

        else:
            server.send_response(player, '✖ No hay un reg-bkp con ese nombre.')

    elif server.is_command(message, 'rb del-bkp'):
        zip = message.removeprefix(f'{server.prefix}rb del-bkp').strip() + '.zip'

        if zip in zips:
            os.remove(os.path.join(reg_bkps_dir, zip))
            show_bkps(server, player)
            server.send_response(player, f'✔ reg-bkp {zip} eliminado.')
        else:
            server.send_response(player, '✖ No hay un reg-bkp con ese nombre.')

    elif server.is_command(message, 'rb list'):
        show_list(server, player)
    
    elif server.is_command(message, 'rb bkps'):
        show_bkps(server, player)

    elif server.is_command(message, 'rb clear'):
        files_to_zip[player] = {'overworld':[],
                                'the_nether':[],
                                'the_end':[]}
        show_list(server, player)

    elif server.is_command(message, 'rb mk-bkp'):
        name = message.removeprefix(f'{server.prefix}rb mk-bkp').strip()
        destination = os.path.join(reg_bkps_dir, f'{name}.zip')
        os.makedirs(os.path.dirname(destination), exist_ok = True)

        if not files_to_zip: 
            server.send_response(player, '✖ No has agregado ninguna región.')
            return
        elif not name: 
            server.send_response(player, '✖ Debes proveer un nombre.')
            return
        elif creating_bkp: 
            server.send_response(player, '✖ Alguien más está creando un backup ahorita.')
            return
        
        creating_bkp = True
        waiting[player] = True

        server.execute('save-off')
        server.execute('save-all')

        while waiting[player]:
            await asyncio.sleep(1)

        server.send_response(player, f'Creando {name}.zip...')
        with zipfile.ZipFile(destination, 'w') as zipf:
            for dim in files_to_zip[player].keys():
                if dim == 'overworld':
                    entities, region, poi = 'entities', 'region', 'poi'
                elif dim == 'the_nether':
                    entities, region, poi = os.path.join('DIM-1','entities'), os.path.join('DIM-1','region'), os.path.join('DIM-1','poi')
                elif dim == 'the_end':
                    entities, region, poi = os.path.join('DIM1','entities'), os.path.join('DIM1','region'), os.path.join('DIM1','poi')
                
                for reg in files_to_zip[player][dim]:
                    file_path = os.path.join(server.path_files, 'world', region, reg)
                    zipf.write(file_path, os.path.join(region, reg))
                    file_path = os.path.join(server.path_files, 'world', poi, reg)
                    zipf.write(file_path, os.path.join(poi, reg))
                    file_path = os.path.join(server.path_files, 'world', entities, reg)
                    zipf.write(file_path, os.path.join(entities, reg))

        server.execute('save-on')
        server.send_response(player, f'✔ reg-bkp {name}.zip creado.')
        server.add_log(f'reg-bkp {name}.zip created')
        creating_bkp = False

async def listener_events(server: Server, log : str):
    global targets, files_to_zip, waiting

    if not 'INFO]:' in log: 
        pass

    elif any(f'{x} has the following entity data' in log for x in targets):
        match = re.search(r"(.*?) has the following entity data: (.*)", log)
        player = match.group(1).strip().split(' ')[-1]
        data = match.group(2)
        raw_pos = data[data.find('Pos:'):]
        raw_pos = raw_pos[raw_pos.find('[') + 1 : raw_pos.find(']')].split(',')
        raw_dim = data[data.find('Dimension:'):]
        dim     = raw_dim[raw_dim.find('"') + 1 : raw_dim.find(',')-1].split(':')[1]
        pos     = tuple(float(x.strip()[:-1]) for x in raw_pos)
        reg     = pos_to_region(pos)

        if not player in files_to_zip.keys(): files_to_zip[player] = {'overworld':[],
                                                                      'the_nether':[],
                                                                      'the_end':[]}

        if not reg in files_to_zip[player][dim]: files_to_zip[player][dim].append(reg)

        show_list(server, player)
        targets.remove(player)
        
    elif log.endswith('Saved the game'):
        for key in waiting.keys():
            waiting[key] = False

def show_list(server: Server, player : str):
    if not player in files_to_zip.keys():
        server.send_response(player, 'Lista vacía.')
        return
    
    msg = []
    i = 0
    for dim in files_to_zip[player].keys():
        for reg in files_to_zip[player][dim]:
            i += 1
            msg .append(f'{i} • {dim} : {reg}')
    
    if len(msg):
        msg.insert(0, f'{player}, region list:')
    else:
        msg = 'Lista vacía.'
    server.send_response(player, msg)

def show_bkps(server: Server, player : str):
    reg_bkps_dir = os.path.join(server.path_plugins, 'reg-bkps')
    zips = [x for x in os.listdir(reg_bkps_dir) if x.endswith('.zip')]
    zips.sort()
    
    if not zips:
        server.send_response(player, 'No se han creado backups.')
        return
    
    server.send_response(player, 'Reg-bkps disponibles:')

    for zip in zips:
        date = datetime.fromtimestamp(os.path.getctime(os.path.join(reg_bkps_dir, zip))).strftime("%Y-%m-%d %H:%M:%S")

        dummy = [
        hover_and_suggest('[>] ' , color = 'green', suggest = f'!!rb load-bkp {zip.removesuffix(".zip")}', hoover = 'Load reg-bkp'),
        hover_and_suggest('[⥁] ', color = 'aqua', suggest = f'!!rb mk-bkp {zip.removesuffix(".zip")}', hoover = 'Remake reg-bkp'),
        hover_and_suggest('[x] ' , color = 'red', suggest = f'!!rb del-bkp {zip.removesuffix(".zip")}', hoover = 'Del reg-bkp'),
        f'{{"text":"{zip} [{date}]"}}'
        ]

        server.execute(f'tellraw {player} {extras(dummy)}')

def pos_to_region(pos : tuple):
    r_x = int(pos[0] // (32*16))
    r_z = int(pos[2] // (32*16))

    return f"r.{r_x}.{r_z}.mca"