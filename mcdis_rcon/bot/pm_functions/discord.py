...
async def confirmation_request( description     : str                              ,*,
    ...
    if interaction:
        await interaction.response.send_message(embed       = discord.Embed(colour = embed_colour, description = description), 
                                                view        = confirmation_views(),
                                                ephemeral   = ephemeral)
    elif channel:
        await channel.send(embed       = discord.Embed(colour = embed_colour, description = description), 
                           view        = confirmation_views())