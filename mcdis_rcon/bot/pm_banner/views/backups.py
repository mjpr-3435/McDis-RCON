...
def backups_views(process: Process) -> discord.ui.View:
    from .files_manager import files_manager_views
    
    pattern = os.path.join(process.path_bkps, f'{process.name} [1-{config["Backups"]}].zip')
    ...
    class views(discord.ui.View):
        ...
        if len(options):
            @discord.ui.select( placeholder = _('Select a backup'), 
                                options = options[:25])
            async def farm_command_selection(self, interaction: discord.Interaction, selection: discord.ui.Select):
                class message_modal(discord.ui.Modal, title = os.path.basename(str(selection.values[0]))):
                    action = discord.ui.TextInput(label = _('Type \'unpack\' or \'delete\' here'), style = discord.TextStyle.short)
                    
                    async def on_submit(modal, interaction: discord.Interaction):
                        if str(modal.action).lower() == _('unpack'):
                            if process.state() in ['Initializing','Open']: 
                                await interaction.response.send_message(content= _('✖ The process must be closed.'), ephemeral=True,delete_after=5)
                                return
                            ...
                            while task.is_alive():
                                if counter[1] == 0 or not isinstance(counter[1], int): 
                                    ...
                                    show = _('`[{}]`: `[{}/{}]` files have been unpacked...').format(process.name, counter[0], counter[1])
                        ...
                        elif str(modal.action).lower() == _('delete'):         
                            try: os.remove(os.path.join(process.path_bkps,str(selection.values[0])))
                            except Exception as error: 
                                await interaction.response.edit_message(content = _('Error: {}').format(error), embed = None, view = None)
                            else:
                                await interaction.response.send_message(_('✔ File deleted.'), ephemeral = True)
                        else:
                            await interaction.response.send_message(content = _('✖ Action `{}` not recognized.').format(str(modal.action).lower()), ephemeral=True)
