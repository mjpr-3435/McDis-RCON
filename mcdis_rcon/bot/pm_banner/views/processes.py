...
def processes_views(path: str, processes: list[psutil.Process], page: int = 1) -> discord.ui.View:
    options = []
    ...
    class views(discord.ui.View):
        ...
        if len(options):
            @discord.ui.select( placeholder = _('Processes'), 
                                options = options[:25])
            async def process_selection(self, interaction: discord.Interaction, selection: discord.ui.Select):
                ...
                await confirmation_request(_('Are you sure about killing the `{}` process?').format(label),
