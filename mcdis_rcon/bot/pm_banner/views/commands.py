...
def commands_views(process: Process) -> discord.ui.View:
    ...
    options.append(discord.SelectOption(label = _('[New Command]'), emoji= emoji_new_command,value = 'New Command'))
    ...
    class views(discord.ui.View):
        ...
        @discord.ui.select( placeholder = _('Select a command'), 
                            options = options[:25])
        async def farm_command_selection(self, interaction: discord.Interaction, selection: discord.ui.Select):
            if selection.values[0] != 'New Command':
                ...
                if len(commands) == 24: 
                    await interaction.response.send_message(_('✖ At the moment, only up to 24 commands are allowed.'), ephemeral = True)
                    return
                
                class message_modal(discord.ui.Modal, title = _('New command')):
                    name = discord.ui.TextInput(label = _('Command name'), style = discord.TextStyle.paragraph)
                
                    async def on_submit(modal, interaction: discord.Interaction):
                        file = f'{str(modal.name)[:40]}.yml'
                        if file in commands:
                            await interaction.response.send_message(_('✖ There is already a command with that name.'), ephemeral = True)
                            return
