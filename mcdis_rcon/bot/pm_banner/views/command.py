...
def command_views(process: Process, file_name: str) -> discord.ui.View:
    class views(discord.ui.View):
        ...
        async def execute_button(self, interaction: discord.Interaction, button: discord.ui.Button):
            if self.process.state() != 'Open':
                await interaction.response.send_message(_('✖ The server isn\'t open.'), ephemeral=True)
                return

            await interaction.response.send_message(_('✔ Executing commands...'), ephemeral=True)
        ...
        async def edit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            class edit_command(discord.ui.Modal, title = _('Edit the command')):
                name = discord.ui.TextInput(label = _('Name'), style = discord.TextStyle.short, default = self.file_name[:-4])
        ...
        async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            await confirmation_request(_('Are you sure about deleting the `{}` command?').format(file_name[:-4]),
