...
def processes_embed(path: str, processes: list[psutil.Process], page : int = 1) -> discord.Embed:
    mrkd_processes = ''
    ...
    embed = discord.Embed(
        title = _('> Processes in `{}`').format(mcdis_path(path)),
        colour = embed_colour,
        description = '```asciidoc\n' + mrkd_processes + '```' if len(mrkd_processes)!= 0 else f'```{_("There are no processes in this folder.")}```')
    if len(mrkd_processes)!= 0: embed.set_footer(text = 184*blank_space + '\n' + _('If you want to close a process, select it from the dropdown below.'))
