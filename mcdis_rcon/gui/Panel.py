...
class ManagerButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(view=McDisManagerView(self.view.client))


class FilesButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
...
class ToolsButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(view=ToolsView(self.view.client))


class GuideButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        embed = discord.Embed(
            ...
            colour=embed_colour,
            description='The guide will be implemented soon, but in the meantime, you can find it on GitHub.'
...
class RestartButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
...
class PanelEmbed(discord.Embed):
    ...
    def _add_resource_fields(self) -> Any:
        ...
        self.add_field(
            inline=True,
            name=self.client._('> Server Resources'),
            value='```asciidoc\nCPU\nFreq::           '[: -len(cpu_freq)]
            + cpu_freq
            + '\nLoad::           '[: -len(cpu_load)]
            + cpu_load
    ...
    def _add_servers_fields(self) -> Any:
        for i, server in enumerate(self.client.servers):
            state = 'Running' if server.is_running() else 'Closed'
            ram_usage = server.ram_usage()
            disk_usage = str(server.disk_usage())

            server_thread = cast(
                discord.Thread,
                next(
                    filter(
                        lambda thread: thread.name.endswith(server.name),
                        self.client.panel.threads,
                    ),
                    None,
                ),
            )
            server_id = server_thread.id

            self.add_field(
                inline=True,
                name=f'> <#{server_id}>',
                value='```asciidoc\nDisk Usage::              '[: -len(disk_usage)]
                + disk_usage
                + '\nRAM Usage::               '[: -len(ram_usage)]
                + ram_usage
                + '\nState::                   '[: -len(state)]
                + state
                + '```',
    ...
    def _add_networks_fields(self) -> Any:
        for i, network in enumerate(self.client.networks):
            ...
            network_thread = cast(
                ...
                next(
                    filter(
                        lambda thread: thread.name.endswith(network.name),
                        self.client.panel.threads,
                    ),
                    None,
                ),
            )
            network_id = network_thread.id

            self.add_field(
                inline=True,
                name=f'> <#{network_id}>',
                value='```asciidoc\nRAM Usage::               '[: -len(ram_usage)] + ram_usage + '```',
            )
