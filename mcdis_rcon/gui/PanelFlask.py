    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        if not self.view.client.flask.is_running:
            ...
            async def on_confirmation(confirmation_interaction: discord.Interaction) -> None:
                shutdown_link = self.view.client.flask.shutdown_link(str(interaction.user))
            ...
            await confirmation_request(
                self.view.client._(
                    'Flask will delay responding to the shutdown request until all '
...
class TemporaryLinksButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(embed=FlaskEmbed(self.view.client), view=self.view)


class OneTimeLinksButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
...
class CleanLinksButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
        super().__init__(
            label='Clean Links',
...
class FlaskEmbed(discord.Embed):
    ...
    def _add_description(self) -> Any:
        ...
        flask_console_id = flask_console.id

        self.add_field(
            inline=True,
            name='',
            value=self.client._(
                'Flask allows you to download files larger than 5 MB. '
                'When you request a file, Flask generates a link that allows you to download it.'
            )
            + f'\n\n<#{flask_console_id}>',
        )

    def _add_status_field(self) -> Any:
        ...
        self.add_field(
            ...
            + '`\n`• State:                  '[: -len(state)]
            + state
            + '`\n`• Links:                  '[: -len(state_uses)]
            + state_uses
            + '`\n`• Links:                  '[: -len(state_time)]
            + state_time
            + '`\n',
        )

    def _add_links_field(self) -> Any:
        content = ''
        ...
        self.add_field(
            inline=False,
            name=self.client._('**Active Links:**'),
            value='' if not content else f'```asciidoc\n{content}```',
        )
