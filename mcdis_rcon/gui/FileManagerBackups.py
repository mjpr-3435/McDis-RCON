...
class BackupsView(discord.ui.View):
    def __init__(self, client: McDisClient, process: Process) -> None:
...
class BackupSelect(discord.ui.Select[Any]):
    def __init__(self, client: McDisClient, options: list[discord.SelectOption]) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if self.view.process.is_running():
            await interaction.response.send_message(
                content=self.view.client._('✖ The process must be closed.'), ephemeral=True
            )
            ...
            async def on_confirmation(confirmation_interaction: discord.Interaction) -> None:
                ...
                make_bkp = self.view.client.error_wrapper(
                    error_title=f'{self.view.process.name}: make_bkp()', reports=reports
                )(self.view.process.make_bkp)

                task = threading.Thread(target=make_bkp, kwargs={'counter': counter})
                task.start()
...
class BackupsEmbed(discord.Embed):
    ...
    def _generate_description(self) -> Any:
        if not self.backups:
