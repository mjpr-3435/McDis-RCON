...
class FileManagerView(discord.ui.View):
    def __init__(self, client: McDisClient, path: str = '.') -> None:
    ...
    def _add_pagination_buttons(self) -> Any:
        if self.max_page > 2:
    ...
    async def _update_embed(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.followup.edit_message(
            message_id=cast(discord.Message, interaction.message).id,
            embed=FileManagerEmbed(self.client, self.path, self.page),
        )

    async def _update_interface(self, interaction: discord.Interaction, back: bool = False) -> None:
        if not interaction.response.is_done():
            await interaction.response.defer()
    ...
    async def _edit_path(self, interaction: discord.Interaction) -> None:
...
class FileSelect(discord.ui.Select[Any]):
    def __init__(self, client: McDisClient, options: list[Any]) -> None:
...
class PathButton(discord.ui.Button[Any]):
...
class FirstPageButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
...
class LastPageButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
        super().__init__(label='>>', style=discord.ButtonStyle.gray, row=2)
        self.view: FileManagerView
...
class PreviousPageButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
        super().__init__(label='<', style=discord.ButtonStyle.gray, row=2)
        self.view: FileManagerView
...
class NextPageButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
        super().__init__(label='>', style=discord.ButtonStyle.gray, row=2)
        self.view: FileManagerView
...
class UpdateButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
...
class RequestButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        if not self.view.client.config['Flask']['Allow']:
            ...
            if not self.view.client.flask.is_running:
                ...
                async def on_confirmation(confirmation_interaction: discord.Interaction) -> None:
...
class EditButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
        ...
        class edit_command(discord.ui.Modal, title=self.view.client._('Edit the file')):
            ...
            async def on_submit(modal, interaction: discord.Interaction) -> None:
                ...
                if file_content:
                    try:
                        ...
                        await self.view.client.error_report(
                            title='Writing File in Files Manager', error=traceback.format_exc()
                        )
                        return
                ...
                try:
                    ...
                    await self.view.client.error_report(
                        title='Renaming File in Files Manager', error=traceback.format_exc()
                    )
                    return
...
class DeleteFileButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
...
class TerminalButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        class message_modal(discord.ui.Modal, title=self.view.client._('Terminal')):
            command: discord.ui.TextInput[Any] = discord.ui.TextInput(
                label=truncate('> ' + mcdis_path(self.view.path), 45),
                style=discord.TextStyle.paragraph,
            )
            example: discord.ui.TextInput[Any] = discord.ui.TextInput(
    ...
    async def _cmd_interface(self, command: str, interaction: discord.Interaction) -> None:
        args = command.split(' ')
        ...
        if handler:
            ...
            await interaction.response.send_message(
                self.view.client._('✖ Invalid command `{}`.').format(command_name), ephemeral=True
            )

    async def _cmd_mkdir(self, args: list[str], interaction: discord.Interaction) -> None:
        if not args:
            await interaction.response.send_message(
    ...
    async def _cmd_zip(self, args: list[str], interaction: discord.Interaction) -> None:
        if not args:
            await interaction.response.send_message(
        ...
        if not isinstance(path_to_zip, str):
            return

        elif not os.path.isdir(path_to_zip):
            await interaction.response.send_message(
                self.view.client._('✖ The path must be a directory.'), ephemeral=True
            )
            return
        ...
        while task.is_alive():
            if counter[1] == 0:
                ...
                show = self.view.client._('`[{}]`: `[{}/{}]` files have been compressed...').format(
                    zip_path, counter[0], counter[1]
                )
    ...
    async def _cmd_unzip(self, args: list[str], interaction: discord.Interaction) -> None:
        if not args:
            await interaction.response.send_message(
        ...
        if not isinstance(path_to_unzip, str):
            return

        elif not os.path.isfile(path_to_unzip) or not path_to_unzip.endswith('.zip'):
            await interaction.response.send_message(
                self.view.client._('✖ The path must be a `.zip` file.'), ephemeral=True
            )
            return
        ...
        await interaction.response.defer()
        response: discord.Message = await interaction.followup.send(
            self.view.client._('Unpacking Backup...'), ephemeral=True, wait=True
        )
        counter = [0, 0]
        reports = {'error': False}
        ...
        while task.is_alive():
            if counter[1] == 0:
                ...
                show = self.view.client._('`[{}]`: `[{}/{}]` files have been unpacked...').format(
                    unzip_path, counter[0], counter[1]
                )
                await response.edit(content=show)
                await asyncio.sleep(0.5)
    ...
    async def _cmd_del(self, args: list[str], interaction: discord.Interaction) -> None:
        ...
        if not isinstance(path_to_remove, str):
            return

        await interaction.response.defer()
        response: discord.Message = await interaction.followup.send(
            self.view.client._('This action might take some time...'), ephemeral=True, wait=True
        )
        await asyncio.sleep(1)
    ...
    async def _cmd_copy(self, args: list[str], interaction: discord.Interaction) -> None:
        if not args[1:]:
            await interaction.response.send_message(
                self.view.client._(
                    '✖ You must provide two arguments. E.g.: `copy <dir:index | file:index> <mcdis_path>`.'
                ),
                ephemeral=True,
            )
            return
        ...
        validation = self.view.client.is_valid_mcdis_path(path_provided, check_if_dir=True)
        new_path = os.path.join(un_mcdis_path(path_provided), os.path.basename(path_to_copy))

        if validation is not True:
            await interaction.response.send_message(validation, ephemeral=True)
            return
        ...
        if reports['error']:
            msg = self.view.client._('✖ An error occurred while copying `{}` to `{}`.').format(
    ...
    async def _cmd_move(self, args: list[str], interaction: discord.Interaction) -> None:
        if not args[1:]:
            await interaction.response.send_message(
                self.view.client._(
                    '✖ You must provide two arguments. E.g.: `move <dir:index | file:index> <mcdis_path>`.'
                ),
                ephemeral=True,
            )
            return
        ...
        if not isinstance(path_to_move, str):
            return
        ...
        await self.view._update_interface(interaction)
        if reports['error']:
            msg = self.view.client._('✖ An error occurred while moving `{}` to `{}`.').format(
    ...
    async def _cmd_rename(self, args: list[str], interaction: discord.Interaction) -> None:
        if not args[1:]:
            await interaction.response.send_message(
                self.view.client._(
                    '✖ You must provide two arguments. E.g.: `rename <dir:index | file:index> <new_name>`.'
                ),
                ephemeral=True,
            )
            return
        ...
        if not isinstance(path_to_rename, str):
            return
    ...
    async def _file_or_dir_selection(
        ...
        if not index.isdigit():
            await interaction.response.send_message(
                self.view.client._('✖ The index must be an integer.'), ephemeral=True
            )
            return None
...
class DeleteDirButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
...
class FileManagerEmbed(discord.Embed):
    def __init__(self, client: McDisClient, path: str = '.', page: int = 1) -> None:
    ...
    def _add_directory_fields(self) -> Any:
    ...
    def _add_file_content(self) -> Any:
        try:
            if not cast(int, get_path_size(self.path, string=False)) > self.max_rqst_size:
