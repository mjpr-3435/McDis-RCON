...
class CommandView(discord.ui.View):
    ...
    def _get_options(self) -> Any:
        ...
        for i, action in enumerate(self.actions):
            options.append(discord.SelectOption(label=action, value=str(i + 1)))

        return options[:25]

    def _get_commands(self) -> list[str]:
        return [str(command) for command in self.data[self.actions[self.action]]]


class ActionSelect(discord.ui.Select[Any]):
...
class ExecuteButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        class edit_command(discord.ui.Modal, title=self.view.client._('Edit the command')):
            ...
            content: discord.ui.TextInput[Any] = discord.ui.TextInput(
                label=self.view.command, style=discord.TextStyle.paragraph, default=yml_content
            )
...
class DeleteButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await confirmation_request(
            self.view.client._('Are you sure you want to delete the `{}` command?').format(
                self.view.command.removesuffix('.yml')
            ),
...
class CommandEmbed(discord.Embed):
    ...
    def _add_description_field(self) -> Any:
        self.add_field(
            inline=False,
    ...
    def _add_action_field(self) -> Any:
