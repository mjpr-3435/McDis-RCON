...
class ProcessesView(discord.ui.View):
    ...
    def _get_options(self) -> Any:
        ...
        for i in range(min_process, min(max_process, len(self.processes))):
            ...
            options.append(discord.SelectOption(label=option_label, value=str(i)))

        return options

    def _update_processes(self) -> Any:
        processes = []
    ...
    async def _update_interface(self, interaction: discord.Interaction) -> None:
        if not interaction.response.is_done():
            await interaction.response.defer()
...
class ProcessSelection(discord.ui.Select[Any]):
...
class ProcessesEmbed(discord.Embed):
    ...
    def _set_descritpion(self) -> Any:
        mrkd_processes = ''
        for i in range(
            self.max_processes * (self.page - 1),
            min(self.max_processes * self.page, len(self.processes)),
        ):
    ...
    def _set_footer(self) -> Any:
        if len(self.processes) != 0:
            ...
            self.set_footer(text=185 * blank_space)
    ...
    def _mrkd(self, process: psutil.Process, index: int) -> str:
        ...
        mrkd_string = (
            f'{index + 1}. {name + blank_space * (20 - len(name))}\n'
