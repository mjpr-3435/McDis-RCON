    ...
    def _get_options(self) -> Any:
        ...
        for file in commands:
            if file.endswith('.yml'):
                options.append(discord.SelectOption(label=file.removesuffix('.yml'), value=file))

        return options[:25]


class CommandSelect(discord.ui.Select[Any]):
        ...
        if self.values[0] != '__NEW_COMMAND__':
            ...
            class message_modal(discord.ui.Modal, title=self.view.client._('New command')):
                name: discord.ui.TextInput[Any] = discord.ui.TextInput(
                    label=self.view.client._('Command name'), style=discord.TextStyle.paragraph
                )
...
class DirButton(discord.ui.Button[Any]):
    def __init__(self, client: McDisClient) -> None:
