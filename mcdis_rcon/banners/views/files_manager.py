...
def files_manager_views(client: McDisClient, path: str = '.') -> discord.ui.View:
    options = []
    up_to_99 = False
    ...
    class views(discord.ui.View):
        ...
        if options:
            @discord.ui.select( placeholder = client._('Select a file'), 
                                options = options[:25])
        ...
        if not os.path.isdir(path):
            ...
            async def request_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                if not client.config['Flask']['Allow']:
                    await interaction.response.defer(ephemeral = True, thinking = True)

                    if get_path_size(path, string = False) > request_size_limit:
                        await interaction.followup.send(client._('✖ McDis only accepts file requests of up to 5MB.'))
                    else:
            ...
            async def edit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                class edit_command(discord.ui.Modal, title = client._('Edit the file')):
                    name = discord.ui.TextInput(label = client._('File'), style = discord.TextStyle.short, default = os.path.basename(path))
                    ...
                    async def on_submit(modal, interaction: discord.Interaction):
                        ...
                        try:
                            ...
                            await interaction.response.send_message(content = client._('Error: {}').format(error), ephemeral=True)
                            return
            ...
            async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                await confirmation_request(client._('Are you sure about deleting the file `{}`?').format(mcdis_path(path)), 
            ...
            async def terminal_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                class message_modal(discord.ui.Modal, title = client._('Terminal')):
                    command = discord.ui.TextInput(label = truncate('> ' + mcdis_path(path), 45), style = discord.TextStyle.paragraph)
                    example = discord.ui.TextInput(label = client._('Commands'), style = discord.TextStyle.paragraph, default = "\n".join(terminal_commands))
            ...
            if not path == '.': 
                ...
                async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                    ...
                    await confirmation_request(client._('Are you sure about deleting the dir `{}`?').format(mcdis_path(path)), 
                                               on_confirmation = on_confirmation,
                                               interaction = interaction)
    ...
    if args[0].lower() == 'mkdir':
        ...
        if len(args) == 1: 
            ...
            if not path_to_unzip: return
                ...
                await interaction.response.send_message(client._('✖ A folder named `{}` already exists.').format(os.path.basename(path_to_unzip[:-4])), ephemeral=True)
                return
            ...
            while task.is_alive():
                if counter[1] == 0 or not isinstance(counter[1], int): 
                    ...
                    show = client._('`[{}]`: `[{}/{}]` files have been unpacked...').format(path_to_unzip[:-4], counter[0], counter[1])
                    await response.edit(content = show)
                    await asyncio.sleep(0.5)
