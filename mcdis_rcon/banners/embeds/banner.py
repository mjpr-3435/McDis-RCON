...
def banner_embed(client: McDisClient) -> discord.Embed:
    ...
    embed = discord.Embed(
            ...
            f'- {client._("Console")} : {" ".join([f"`{command}`" for command in console_commands])}' + '\n'
            f'- {client._("Panel")  } : {" ".join([f"`{mcdis_prefix}{command}`" for command in panel_commands])}' )
    
    
    embed.add_field(name = client._('> Server Resources'), inline = True, value =
            f'```asciidoc\n'
            f'CPU\n'
    ...
    for server in client.servers:
        ...
        embed.add_field(
            inline = True, 
            name = f'> <#{next(filter(lambda thread: server.name in thread.name, client.panel.threads)).id}>',
            value = 
                f'```asciidoc\n'
    ...
    if len(client.servers)%2 == 1:
        embed.add_field(name = '', value = '', inline = True)
    
    embed.add_field(name = '', value = '', inline = False)
          
    for process in client.networks:
        ram_usage = process.ram_usage()

        embed.add_field(
            inline = True, 
            name = f'> <#{next(filter(lambda thread: process.name in thread.name, client.panel.threads)).id}>',
            value = 
                f'```asciidoc\n'
                f'RAM Usage::               '[:-len(ram_usage)] + ram_usage + '```')
    
    if len(client.networks)%2 == 0:
        embed.add_field(name = '', value = '', inline = False)
    ...
    if warnings:
        embed.add_field(name = client._('> Warnings'), inline = False, value = '\n'.join([f'`{warning}`' for warning in warnings]))
    embed.url = link_repository
    return embed