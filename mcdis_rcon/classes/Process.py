...
class Process():
    def __init__(self, name: str, client: McDisClient, process_config: dict):
        self.name                   = name
        self.client                 = client
        ...
        self.mcdis_config           = client.config
    ...
    def         start               (self):
        ...
        try:
            ...
            asyncio.create_task(self.error_report(self.client._('Error in the function start().\n\n{}').format(traceback.format_exc())))
            self.stop()
        ...
        if modules:
            ...
            for module in modules:
                try:
                    ...
                    asyncio.create_task(self.error_report(self.client._('Error while trying to load the plugin {}.\n\n{}').format(module,traceback.format_exc())))
    ...
    async def   call_plugins        (self, function: str, args: tuple):
       for plugin in self.plugins:
            try: 
                ...
                await func(*args)
            except: await self.error_report(self.client._('Error in {}() of {}.\n\n{}').format(function, plugin, traceback.format_exc()))
    ...
    async def   error_report        (self, error : str):
        mrkd_error = f'```diff\n- Error Report:\n{error[:1950]}\n```'
        error_reports = await thread('Error Reports', self.client.panel)
    ...
    async def   listener_console    (self):
        ...
        async def relay_console(self: Process):
            ...
            while self.process.poll() is None or (not self.console_relay.empty()) and not self.stop_relay:
                try:
                    ...
                    await self.error_report(self.client._('Error in the function relay_console().\n\n{}').format(traceback.format_exc()))
        ...
        try: 
            ...
            await self.error_report(self.client._('Closing Process.\nError in the function listener_console().\n\n{}').format(traceback.format_exc()))
            return
