...
from ..modules import package_path
from ..utils import mcdis_path, thread, truncate
...
class FlaskManager(Flask):
    def __init__(self, client: McDisClient) -> None:
        ...
        self.active_downloads: dict[str, dict[str, str]] = {}
        self.shutdown_requests: dict[str, dict[str, str]] = {}
        self.is_running = False
        self.temporary_links = True
        self.one_time_links = True
        self.ip = client.config['Flask']['IP'] if client.config['Flask']['Allow'] else '——'
        self.port = client.config['Flask']['Port'] if client.config['Flask']['Allow'] else 0
        ...
        #   self.add_url_rule(
        #       '/upload', view_func=self._upload_file, methods=['POST']
        #   )
        #   self.add_url_rule('/selection'      , view_func = self._upload_form)
        self.handle_http_exception = self._handler  # type: ignore[assignment, method-assign]

    def _handler(self, exception: Any) -> None:
        if exception.code == 404:
            log = f'Unusual connection attempt to: {request.path} with args: {request.args}'
            self._get_log_queue().put(self.log_format(log))
        return

    def _favicon(self) -> Any:
        return send_from_directory(
            os.path.join(package_path, 'extras'), 'mcdis.ico', mimetype='image/vnd.microsoft.icon'
        )

    def _download_file(self) -> Any:
    ...
    def _shutdown(self) -> Any:
        id = request.args.get('id')
        if id in self.shutdown_requests:
    ...
    def _upload_file(self) -> Any:
        ...
        if file:
            ...
            file.save(filepath)
            return f'File {filename} uploaded to {mcdis_path(dirpath)}'

    def _is_valid_addres(self) -> bool:
        try:
            ...
            asyncio.create_task(
                self.client.error_report(
                    title='Flask: Invalid Adress', error=traceback.format_exc()
                )
            )
            return False
        else:
    ...
    def start(self) -> None:
        if self.is_running or not self._is_valid_addres():
            return
    ...
    def shutdown_link(self, user: str) -> str:
    ...
    async def _remove_link(self, id: str) -> None:
    ...
    def download_link(self, file_path: str, user: str) -> str:
        id = self._generate_id()

        while id in self.active_downloads:
            id = self._generate_id()

        self.active_downloads[id] = {'user': user, 'file': file_path}
        if self.temporary_links:
    ...
    def clean_active_links(self) -> None:
        if not self.is_running:
            return

        self.active_downloads = {}
        log = 'Active links have been removed.'
        self._get_log_queue().put(self.log_format(log))

    def log_format(self, log: str, type: str = 'INFO') -> str:
        return f'[McDis] [Flask] [{datetime.now().strftime("%H:%M:%S")}]: {log}'

    def _get_log_queue(self) -> queue.Queue[str]:
        if self._log_queue is None:
            raise RuntimeError('Flask log queue is not initialized.')
        return self._log_queue

    async def send_to_console(self, message: str) -> None:
        mrkd = f'```md\n{truncate(message, 1990)}\n```'
        remote_console = await thread('Console Flask', self.client.panel)
...
def _upload_form(self: Any) -> str:  # pyright: ignore[reportUnusedFunction]
    return """
        <!doctype html>
        <title>Subir Archivo</title>
...
def _upload_file(self: Any) -> Any:  # pyright: ignore[reportUnusedFunction]
    if 'file' not in request.files:
        return abort(404)
    file = request.files['file']
    if file.filename == '':
